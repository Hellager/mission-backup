<template>
    <div class="container">
        <page-header :title="t('component.pageHeaderStatisticPage')" to="/" />
        <div class="select">
        ...
        <v-chart class="chart" id="statistic" :option="option" :autoresize="true" theme="light"></v-chart>
...
function form_x_axis_with_datetime(from: string, length: number):Array<string> {
  ...
  for (let i = 0; i < length; i++) {
    let combined_datetime = month + "/" + (parseInt(day) + i).toString().padStart(2, "0");
...
function change_date_picker_format(val: any) {
  let current_time_type = selected_query_time_type.value;
    ...
    await execute_rust_command(TauriCommand.COMMAND_GET_MISSION_BACKUPS_STATUS, 
          ...
          .then((res) => {
            if (res.code === 200) {
              ...
              let updated_xAxis = [];
              let value_length = res.data.count.length;
