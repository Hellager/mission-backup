            ...
            <el-table 
              ...
              @row-dblclick="toggle_row_db_click">
                ...
                <el-table-column :label="t('table.status')">
                  <template #default="props">
                      <div class="statusContainer">
                        <el-icon class="loading" v-if="props.row.info.status === 'executing'"><Loading /></el-icon> 
                        <el-icon class="status" v-if="props.row.info.status === 'timing'"><Clock /></el-icon> 
                        <el-icon class="status" v-if="props.row.info.status === 'monitoring'"><View /></el-icon> 
                        <el-icon class="status" v-if="props.row.info.status === 'pausing'"><VideoPause /></el-icon> 
                        <el-icon class="status" v-if="props.row.info.status === 'unavailable'"><Warning /></el-icon> 
                ...
                <template v-slot:empty>
                  <el-empty :description="t('table.empty')"></el-empty>
                </template>
            </el-table>       
        </div>
...
import { VideoPause, VideoPlay, Edit, Delete, Loading, View, Clock, Warning } from '@element-plus/icons-vue';
...
async function toggle_delete_dlick(id: string) {
  let current_mission_status = missionStore.get_mission_status(id);
