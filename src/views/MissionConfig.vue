<template>
    <div class="container">
        <page-header :title="t('component.pageHeaderMissionConfig')" to="/" />
            ...
            <el-form
                ...
                <el-form-item :label="t('config.type')" prop="target_type">
                    <el-radio-group v-model="form.target_type" :disabled="form.from_path != ''">
                        <el-radio label="file" size="large">{{ t("config.file") }}</el-radio>
                        <el-radio label="directory" size="large">{{ t("config.directory") }}</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item :label="t('config.fromPath')" prop="from_path">
                    <div class="select">
                        ...
                        <el-button type="primary" :disabled="route.query.mode == 'edit'" @click="select_path('from', 'mission')">{{ t("config.select") }}</el-button>                        
                    </div>
                </el-form-item>
                <el-form-item :label="t('config.toPath')" prop="to_path">
                    <div class="select">
                        ...
                        <el-button type="primary" @click="select_path('to', 'mission')">{{ t("config.select") }}</el-button>
                    </div>
                </el-form-item>
                ...
                <transition name="fade" enter-active-class="animate__animated animate__fadeInUp" leave-active-class="animate__animated animate__fadeOutDown">
                    <el-form-item :label="t('config.ignoreMethod')" v-if="form.ignore_enable">
                        <el-radio-group v-model="form.ignore_method">
                            <el-radio label="custom" size="large">{{ t('config.selectIgnore') }}</el-radio>
                            <el-radio label=".gitignore" size="large">{{ t('config.hasGitignore') }}</el-radio>
                        </el-radio-group>   
                    </el-form-item>                
                </transition>
                <transition name="fade" enter-active-class="animate__animated animate__fadeInUp" leave-active-class="animate__animated animate__fadeOutDown">
                    <el-form-item :label="t('config.selectIgnore')" v-if="form.ignore_method === 'custom' && form.ignore_enable">
                        <div class="select">     
                            ...
                            <el-button type="primary" @click="select_path('', 'ignore_file')">{{ t('config.selectIgnoreFiles') }}</el-button>
                            <el-button type="primary" @click="select_path('', 'ignore_folder')">{{ t('config.selectIgnoreFolders') }}</el-button>
                        </div>     
                    </el-form-item>
                </transition>
                ...
                <transition name="fade" enter-active-class="animate__animated animate__fadeInUp" leave-active-class="animate__animated animate__fadeOutDown">
                    <el-form-item :label="t('config.compressFormat')" v-if="form.compress_enable">
                ...
                <transition name="fade" enter-active-class="animate__animated animate__fadeInUp" leave-active-class="animate__animated animate__fadeOutDown">
                    <el-form-item :label="t('config.cronExpression')" v-if="form.cron_enable" prop="cron_expression">
                        <el-tooltip :content="cron_string" :trigger-keys="[]" :disabled="!form.cron_enable || cron_string == ''" :show-after="2000" placement="top" effect="light">
                            <el-input class="select" v-model.trim="form.cron_expression" :disabled="!form.cron_enable" :placeholder="t('config.inputCron')" />
                        </el-tooltip>                    
                    </el-form-item>
                </transition>
                    ...
                    <div class="restrict">
                        ...
                        <!-- <transition name="fade" enter-active-class="animate__animated animate__fadeInUp" leave-active-class="animate__animated animate__fadeOutDown"> -->
                            <div class="restrict_item">
                                <el-input-number v-model="form.save_days" v-if="form.restrict_save_days_enable" :min="1" :max="365" @change="handle_change_save_days" />                        
                                <span class="item_label" v-if="form.restrict_save_days_enable">{{t('config.days')}}</span>
                            </div>
                        <!-- </transition> -->
                    </div>
                </el-form-item>
                <el-form-item :label="t('config.saveSizeEnalbe')">
                    <div class="restrict">
                        ...
                        <!-- <transition appear name="fade" enter-active-class="animate__animated animate__fadeInRight" leave-active-class="animate__animated animate__fadeOutRight"> -->
                            <div class="restrict_item">
                                <el-input-number v-model="form.save_size" v-if="form.restrict_save_size_enable" :min="1" :max="1024" @change="handle_change_save_size" />
                                <span class="item_label" v-if="form.restrict_save_size_enable">{{t('config.size')}}</span>
                            </div>
                        <!-- </transition> -->
                    </div>
                </el-form-item>
                ...
                <el-form-item>
                    <el-button type="primary" @click="submit_form(missionFormRef)">{{ t('config.confirm') }}</el-button>
...
import type { MissionConfig, Mission } from '../store/modules/mission/types';
...
const validate_cron = (rule: any, value: any, callback: any) => {
    ...
    if (value === '') {
        ...
        if (form.cron_expression !== '') {
            try {
                let cron_locale = locale.value === 'zh-CN'? 'zh_CN' : 'en';
...
async function check_exists_before_submit() {
    ...
    for (let i = 0; i < check_list.length; i++) {
        let mission = check_list[i];
        if (mission.config.id === form.id && route.query.mode === 'edit') continue;
...
async function check_path_valid() {
    let isFromPathValid = await execute_rust_command(TauriCommand.COMMAND_CHECK_PATH_VALID, form.from_path, form.target_type)
            .then(res => {
                if (!res) {
                    ElMessage.error({
                        showClose: true,
    ...
    let isToPathValid = await execute_rust_command(TauriCommand.COMMAND_CHECK_PATH_VALID, form.to_path, "directory")
            .then(res => {
                if (!res) {
                    ElMessage.error({
                        showClose: true,
...
async function check_valid() {
    let isExists = await check_exists_before_submit();
    let isPathValid = await check_path_valid();
